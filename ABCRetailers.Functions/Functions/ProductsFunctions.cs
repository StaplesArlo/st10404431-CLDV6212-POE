        ...
        [HttpTrigger(AuthorizationLevel.Anonymous, "put", Route = "products/{id}")] HttpRequestData req, string id)
    {
        ...
        try
        {
            ...
            if (contentType.StartsWith("multipart/form-data", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (form.Text.TryGetValue("StockAvailable", out var stockTxt) && int.TryParse(stockTxt, out var stock)) e.Stock = stock;
            ...
            else
            {
                ...
                if (body.TryGetValue("StockAvailable", out var st) && int.TryParse(st.ToString(), out var stock)) e.Stock = stock;
